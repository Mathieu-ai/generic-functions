 ...
 * @returns Object | data
 */
export async function ml_API(env: string): Promise<object[]> {
...
export const mlFn = {
     ...
     * // ]
     */
    sort: ({ arr, prop }: { arr: any[]; prop: string }): object[] => {
     ...
     */
    flat: (obj: object | object[] | number | string | boolean, keys: string[] = []): string => {
     ...
     * // { "name": { "common": "France", "official": "French Republic" }, "cca2": "FR" }
     */
    GetCountry: ({ cc, cn }: { cc?: string; cn?: string }, countries: any[]): object | undefined => {
        const searchInAltNames = (altNames: string) => {
            if (!altNames) return false;
            return altNames.split(',').some((alt: any) => alt.trim() === mlFn.simplify(cn));
        };
        ...
        return countries
            ...
            .find(
                ({ name, altNames, cca2 }) =>
                    mlFn.simplify(name.common) === mlFn.simplify(cn) || mlFn.simplify(name.official) === mlFn.simplify(cn) || searchInAltNames(mlFn.simplify(altNames)) || cca2 === cc
            );
    }
};
...
//  * const result = addState({ obj, tbState: companies });
//  * console.log(result)
//  * // { state: "California", bgColor: "red", icon: "CA", type: "state" }
//  */
// export function addState({ obj, tbState }: { obj: any; tbState: any[] }) {
